...
int main( int argc, char** argv )
{
  ...
  general_opt.add_options()
    ...
    ("zoom,z", po::value<int>()->default_value( 2 ), "Specifies the zoom factor for the output superresolution image.")
    ("lambda,l", po::value<double>()->default_value( 1 ), "The data fidelity term in TV denoising (10: very high, 0.01: very low" ) 
    ("theta,T", po::value<double>()->default_value( 0.1 ), "The theta term that fits the denoised u to the data (should be small to be a good approximation of the inpainting." ) 
  ...
  if( ! parseOK || vm.count("help") || argc <= 1 || (! vm.count( "input") ) )
    {
      trace.info()<< "Computes a kind of superresolution image by using Total Variation regularization of a color ppm or gray-level pgm image. It is an implementation of Li, Bao, Liu, Zhang variant of Chambolle, Pock primal-dual algorithm." <<std::endl << "Basic usage: " << std::endl
		  << "\ttv-zoom-image [options] -i <image.ppm> -l 1 -T 0.1 "<<std::endl
		  << general_opt << "\n";
      return 0;
    }
  ...
  if ( color ) {
    typedef ImageTVZoom<Space, 3> ColorTV;
    ColorTV tv;
    tv.init( image, ColorTV::Color2ValueFunctor(), zoom );
    tv.optimize( lambda, theta, dt, tol, max_iter );
    if ( out_color ) tv.outputU( output_u, ColorTV::Value2ColorFunctor() );
    else             tv.outputU( output_u, ColorTV::Value2GrayLevelFunctor() );
  } else {
    typedef ImageTVZoom<Space, 1> GrayLevelTV;
    GrayLevelTV tv;
    tv.init( image, GrayLevelTV::GrayLevel2ValueFunctor(), zoom );
    tv.optimize( lambda, theta, dt, tol, max_iter );
